...
The mechanical half of those rules is checked by CI, and locally by `npx companygraph-meta-model check`. What no check reads is each schema's `## Writing rules`, which an agent judges by reading them against the entity.
